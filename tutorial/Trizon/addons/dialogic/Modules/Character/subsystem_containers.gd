...
var transform_regex: = "(?<part>position|pos|size|siz|rotation|rot)\\W*=(?<value>((?!(pos|siz|rot)).)*)"
...
func str_to_vector(input: String, base_vector: = Vector2()) -> Vector2:
    var vector_regex: = RegEx.create_from_string("(?<part>x|y)\\s*(?<number>(-|\\+)?(\\d|\\.|)*)(\\s*(?<type>%|px))?")
    var vec: = base_vector
