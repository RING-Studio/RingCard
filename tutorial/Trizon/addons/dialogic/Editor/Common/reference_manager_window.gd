...
func add_ref_change(old_name: String, new_name: String, type: Types, where: = Where.TEXTS_ONLY, character_names: = [], 
    ...
    match type:
        ...
        Types.VARIABLE:
            regexes = [
                "{(?<replace>\\s*%s\\s*)}" % old_name.replace("/", "\\/"), 
                "var\\s*=\\s*\"(?<replace>\\s*%s\\s*)\"" % old_name.replace("/", "\\/")
                ]
        ...
        Types.PORTRAIT:
            regexes = [
                "(?m)^[^:(\\n]*\\((?<replace>%s)\\)" % old_name.replace("/", "\\/"), 
                "\\[\\s*portrait\\s*=(?<replace>\\s*%s\\s*)\\]" % old_name.replace("/", "\\/")
                ]
        ...
        Types.CHARACTER_NAME:
            ...
            regexes = [
                "((join|leave|update) )?(?<replace>%s)(?!\\B)(?(1)|(?!([^:\\n]|\\\\:)*(\\n|$)))" % old_name
                ]
            category_name = "Renamed Character Files"

        Types.TIMELINE_NAME:
            regexes = [
                "timeline ?= ?\" ?(?<replace>%s) ?\"" % old_name
                ]
