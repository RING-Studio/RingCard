...
func get_events_indexed(events: Array) -> Dictionary:
    ...
    for event in events:
        ...
        if "end_node" in event and event.end_node:
            ...
            if event.parent_node in events:
...
func copy_selected_events() -> void :
    ...
    for item in selected_items:
        ...
        if item.resource is DialogicEndBranchEvent:
            if item.parent_node in selected_items:
...
func move_blocks_to_index(blocks: Array, index: int):
    ...
    for event in blocks:
        ...
        undo_indexes[index - index_shift + event_count] = event.get_index() + index_shift * int(index < event.get_index())
        event_count += 1
...
func _input(event: InputEvent) -> void :
    ...
    match event.as_text():
        ...
        "Ctrl+Shift+T", "Ctrl+Alt+T", "Ctrl+Option+T":
            get_viewport().set_input_as_handled()
